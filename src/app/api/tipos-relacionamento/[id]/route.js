...
export async function PUT(request, { params }) {
  try {
    ...
    const tipo = await TipoDeRelacionamento.findByIdAndUpdate(id, body, { new: true, runValidators: true });
    if (!tipo) {
...
export async function DELETE(request, { params }) {
  try {
    ...
    const resolvedParams = await params;
    const { id } = resolvedParams;
    const result = await TipoDeRelacionamento.findByIdAndDelete(id);
    if (!result) {
      return NextResponse.json({ success: false, error: 'Tipo não encontrado' }, { status: 404 });
    }
